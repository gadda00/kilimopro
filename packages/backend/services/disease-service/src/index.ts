/**
 * Disease Service
 * Microservice for crop disease detection and management
 */
...
const logger = getLogger('disease-service');
...
async function start() {
  ...
  await app.register(swagger, {
    swagger: {
      info: {
        title: 'KilimoPRO Disease Service',
        description: 'Microservice for crop disease detection, management, and alerts',
        version: '1.0.0',
      },
  ...
  // API Routes
  await app.register(detectRoutes, { prefix: '/api/disease' });
  await app.register(modelRoutes, { prefix: '/api/disease' });
  await app.register(historyRoutes, { prefix: '/api/disease' });
  await app.register(alertRoutes, { prefix: '/api/disease' });

  // Start server
  try {
    await app.listen({ port: config.port, host: config.host });
    logger.info(`Disease service running on ${config.host}:${config.port}`);

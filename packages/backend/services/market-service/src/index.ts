/**
 * Market Service
 * Microservice for market price data and analysis
 */
...
const logger = getLogger('market-service');

// Initialize clients
const db = getDatabaseClient('market-service');
const cache = getCacheClient('market-service');
const mq = getMessageQueueClient('market-service');
...
async function start() {
  ...
  await app.register(swagger, {
    swagger: {
      info: {
        title: 'KilimoPRO Market Service',
        description: 'Microservice for market price data and analysis',
        version: '1.0.0',
      },
  ...
  // API Routes
  await app.register(priceRoutes, { prefix: '/api/market' });
  await app.register(trendRoutes, { prefix: '/api/market' });
  await app.register(forecastRoutes, { prefix: '/api/market' });
  await app.register(marketRoutes, { prefix: '/api/market' });

  // Start server
  try {
    await app.listen({ port: config.port, host: config.host });
    logger.info(`Market service running on ${config.host}:${config.port}`);
